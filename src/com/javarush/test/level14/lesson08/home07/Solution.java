...
public class Solution
{
    ...
    public static void cleanAllApartaments(List<Apartament> apartaments)
    {
        ...
        for (Apartament forAp : apartaments)
        {
            if (forAp instanceof Apt1Room)
            {
                ((Apt1Room) forAp).clean1Room();
                continue;
            }
            if (forAp instanceof Apt2Room)
            {
                ((Apt2Room) forAp).clean2Rooms();
                continue;
            }
            if (forAp instanceof Apt3Room)
                ((Apt3Room) forAp).clean3Rooms();
        }
